...
export default function LoginPage(): JSX.Element {
  ...
  return (
    <div className="container login-page__full-height-container is-fullhd is-justify-content-center is-align-items-center is-flex">
      <div className="box login-panel__panel-container">
        <h1 className="title has-text-centered is-uppercase">Авторизация</h1>
        <div className="tabs is-centered is-fullwidth">
          <ul>
            {tabs.map(({id, typeTab, nameTab}: {id: number; typeTab: string; nameTab: string}) => {
              return (
        ...
        <div className="login-panel__auth-form">{stateTabs === 'login' ? <AuthLoginForm /> : <AuthCryptoForm />}</div>
      </div>
    </div>
  );
}
